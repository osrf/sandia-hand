...
void comms_init()
{
  ...
  g_comms_rs485_address = *((uint32_t *)0x0401ffc); // defined in bootloader
}
                          ...
                          uint16_t pkt_crc)
{
  ...
  {
    imu_power(payload[0] & 0x01);
    //strain_power(payload[0] & 0x02);
  ...
  {
    // todo: update this. scan the thermistors and ARM/IMU temp sensors
    //thermal_scan(g_tx_pkt_buf+5);
    for (int i = 0; i < 8; i++)
      g_tx_pkt_buf[5+i] = 0x42;
